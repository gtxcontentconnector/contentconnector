 ...
 *  spellchecker
 *  	.indexDictionary(new LuceneDictionary(my_lucene_reader, a_field));
 *  // To index a file containing words:
 *  spellchecker
 *  	.indexDictionary(new PlainTextDictionary(new File("myfile.txt")));
...
public class CustomSpellChecker implements java.io.Closeable {
   ...
   * @param xsd the {@link StringDistance} measurement to use 
   * @throws IOException if Spellchecker can not open the directory
   */
  public CustomSpellChecker(final Directory sspellIndex,
  ...
  public final void setSpellIndex(
		  final Directory spellIndexDir) throws IOException {
   ...
   * 
   * @param xsd the {@link StringDistance} implementation for this
   * {@link SpellChecker} instance
   */
   ...
   * matching spell-checked word from the hits that Lucene found, 
   * one usually has
   ...
   * @return String[]
   */
  public final String[] suggestSimilar(final String word,
		  final int numSug) throws IOException {
   ...
   * 
   * <p>As the Lucene similarity that is used to fetch the 
   * most relevant n-grammed terms
   * is not the same as the edit distance strategy used to calculate the best
   * matching spell-checked word from the hits that Lucene 
   * found, one usually has
   * to retrieve a couple of numSug's in order to get the true best match.
   *
   * <p>I.e. if numSug == 1, don't count on that suggestion being the best one.
   * Thus, you should set this value to <b>at least</b> 5 for a good suggestion.
   ...
   * @param field the field of the user index: 
   * if field is not null, the suggested
   * words are restricted to the words present in this field.
   * @param morePopular return only the suggest 
   * words that are as frequent or more frequent than the searched word
   * (only if restricted mode = (indexReader!=null and field!=null)
   * @throws IOException if the underlying index throws an {@link IOException}
   * @return String[] the sorted list of the suggest words 
   * with these 2 criteria:
   * first criteria: the edit distance, second criteria 
   * (only if restricted mode): the popularity
   * of the suggest words in the field of the user index
   */
  public final String[] suggestSimilar(final String word,
		  final int numSug, final IndexReader ir,
      final String field, final boolean morePopular) throws IOException {
  ...
  //    System.out.println("HITS: " + hits.length());
      ...
      for (int i = 0; i < stop; i++) {
        ...
        if (sugWord.getString().equals(word)) {
          log.debug("  Found word is the same as input word (" 
        		  + word + ") -> next");
          continue;
        }
        ...
        if (sugWord.getScore() < min) {
          log.debug("  Found word does not match min score (" 
        		  + min + ") -> next");
          continue;
        }
   ...
   * @param boost boost
   */
  private static void add(final BooleanQuery q,
		  final String name, final String value, final float boost) {
  ...
  private static void add(final BooleanQuery q,
    ...
    q.add(new BooleanClause(new TermQuery(
    		new Term(name, value)), BooleanClause.Occur.SHOULD));
  }
   ...
   * @return an array of all ngrams 
   * in the word and note that duplicates are not removed
   */
  ...
  public final void clearIndex() throws IOException {
    synchronized (modifyCurrentIndexLock) {
      ...
      final IndexWriter writer = new IndexWriter(dir, null,
   ...
   * @throws IOException in case of error
   */
  public final void indexDictionary(final Dictionary dict, 
		  final int mergeFactor, final int ramMB) throws IOException {
    synchronized (modifyCurrentIndexLock) {
      ensureOpen();
      final Directory dir = this.spellIndex;
      ...
      writer.optimize();
      writer.close();
      // also re-open the spell index to 
      // see our own changes when the next suggestion
      // is fetched:
      swapSearcher(dir);
    }
  }
  ...
  /**
   * Release Searcher.
   * @param aSearcher searcher
   * @throws IOException in case of error.
   */
  private void releaseSearcher(final 
