...
public class WhitespaceFragmentsBuilder implements FragmentsBuilder {
	 ...
	 */
	public WhitespaceFragmentsBuilder(final String[] prTags
			, final String[] poTags) {
		this.preTags = prTags;
	...
	protected final String getFragmentSource(final StringBuilder buffer, 
			final int[] index, final Field[] values, 
			final WeightedFragInfo fragInfo) {
	 ...
	 * @return test
	 */
	public final String createFragment(final IndexReader reader, 
			final int docId, final String fieldName,
			final FieldFragList fieldFragList) 
					throws IOException {
		    String[] fragments = createFragments(reader, docId, fieldName,
		    		fieldFragList, 1);
		 ...
		 * @return fragments
		 */
		  public final String[] createFragments(final IndexReader reader, 
				final int docId, final String fieldName, 
				final FieldFragList fieldFragList, final int maxNumFragments)
		      throws IOException {
		    if (maxNumFragments < 0) {
		      throw new IllegalArgumentException("maxNumFragments(" 
		    		  + maxNumFragments + ") must be positive number.");
		    }
		    List<WeightedFragInfo> fragInfos = 
		    		getWeightedFragInfoList(fieldFragList.fragInfos);
		    ...
		    for (int n = 0; n < maxNumFragments && n < fragInfos.size(); n++) {
		      ...
		      fragments.add(makeFragment(buffer, nextValueIndex, 
		    		  values, fragInfo));
		    }
		  ...
		  protected final String makeFragment(final StringBuilder buffer, 
			   ...
			   * @return fragment
			   */
			  private String makeFragment(final WeightedFragInfo fragInfo,
					final String src, final int s) {
			    StringBuilder fragment = new StringBuilder();
			    int srcIndex = 0;
			    for (SubInfo subInfo : fragInfo.subInfos) {
			      for (Toffs to : subInfo.termsOffsets) {
			        fragment.append(src.substring(srcIndex, to.startOffset - s))
			        		.append(getPreTag(subInfo.seqnum))
			        		.append(src.substring(to.startOffset - s, 
			        				to.endOffset - s)).append(
			        				getPostTag(subInfo.seqnum));
			        srcIndex = to.endOffset - s;
			      }
			    }
			    fragment.append(src.substring(srcIndex));
		 ...
		 */
		protected final Field[] getFields(final IndexReader reader, 
