...
public final class ImageResizer {
	 ...
	 * cache region.
	 */
	private static final String IMAGE_CACHE_REGION 
		= "gentics-portal-contentmodule-image";
	...
	private static JCS initCache() {
		...
		try {
			...
			log.debug("Using cache zone " + IMAGE_CACHE_REGION 
					+ " for image resizing");
	 ...
	 */
	public static byte[] getResizedImage(final CRResolvableBean bean,
	...
	public static byte[] getResizedImage(final CRResolvableBean bean, 
        ...
        if (maxw > MAX_ALLOWED_WIDTH) {
            log.warn("Width {" + maxw
                    ...
                    + MAX_ALLOWED_WIDTH + "}, using the maxallowedwidth "
                    + "instead!");
        ...
        if (maxh > MAX_ALLOWED_HEIGHT) {
            log.warn("Height {" + maxh
                    ...
                    + MAX_ALLOWED_HEIGHT + "}, using the maxallowedheight "
                    + "instead!");
        ...
        if (maxw > 0 || maxh > 0) {
            ...
            final String cacheKey = bean.getContentid() + "." 
            	+ bean.get("updatetimestamp") 
            	+ "." + maxWidth + "." + maxHeight; 
	 ...
	 */
	private static byte[] resizeImage(final CRResolvableBean motherObj, 
        ...
        try {
            ...
            if (binary == null) {
                ...
                log.error("Unable to find binarycontent for object {" 
                		+ motherObj.getContentid() + "} to resize image.");
                return null;
            }
            ...
            // do the resizing
            binary = GenticsImageResizer.resize(binary, Math.max(maxwidth, 0),
