...
public final class Instanciator {
    ...
    for (Object[] parameters : prioritizedParameters) {
      ...
      try {
        ...
        logger.debug("Cannot instanciate object for class " + clazz + " with"
            + "parameters (" + getReadableStringFromClassArray(parameterClasses)
            + ").", e);
  ...
  /**
   * Tries to find a {@link Constructor} of the given {@link Class} that matches
   * the given parameters types.
  ...
  /**
   * tests if the given source parameter types would match the target parameter
   * types.<br /> this can be used to test if a {@link Constructor} with the
   * target parameters can be called with the source parameters.
   * @param targetParameterTypes parameters of the {@link Constructor}
   * @param sourceParameterTypes parameters to call the {@link Constructor} with
      ...
      for (int i = 0; i < targetParameterTypes.length; i++) {
        if (sourceParameterTypes[i] == null) {
          ...
          if (primitiveParameterType != null
              && primitiveParameterType.equals(targetParameterTypes[i])) {
             matches++;
          }
          ...
          if (primitiveParameterType != null
              && primitiveParameterType.equals(sourceParameterTypes[i])) {
             matches++;
          }
        }
      }
