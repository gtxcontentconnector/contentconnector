...
public class MergeTransformer extends ContentTransformer {
	...
	public MergeTransformer(final GenericConfiguration config) {
		...
		if (sourceAttributes == null) {
			logger.error("Please configure " + TRANSFORMER_SOUCREATTRIBUTE_KEY + " for my config.");
		}
		...
		if (sourceAttributes != null && targetAttribute != null) {
			...
			for (String attribute : sourceAttributes) {
				if (attribute.startsWith(quot) && attribute.endsWith(quot)) {
					target.append(attribute.substring(quot.length(), (attribute.length() - (2 * quot.length()))));
