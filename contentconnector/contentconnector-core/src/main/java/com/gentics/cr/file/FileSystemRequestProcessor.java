...
public class FileSystemRequestProcessor extends RequestProcessor {
  ...
  private static Logger logger =
    Logger.getLogger(FileSystemRequestProcessor.class);
  ...
  public final Collection<CRResolvableBean> getObjects(final CRRequest request,
    ...
    try {
      Expression expressionFromFilter =
        ExpressionParserHelper.parse(requestFilter);
      for (CRResolvableBean item : new Vector<CRResolvableBean>(
          monitoredPath.getRegisteredDescendants())) {
      ...
      logger.error("Cannot parse filter " + requestFilter
            + " to a valid expression.", e);
    } catch (ExpressionParserException e) {
      logger.error("Filter " + requestFilter
          + " doesn't results into a boolean.", e);
    }
  ...
  private ResolvableFileBean getResolvableFileBeanForPath(final String path) {
    ...
    ResolvableFileBean resolvableFileBeanForPath =
      new ResolvableFileBean(fileForPath);
