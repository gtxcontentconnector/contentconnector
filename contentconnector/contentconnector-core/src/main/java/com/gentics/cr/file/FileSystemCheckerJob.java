...
public class FileSystemCheckerJob extends Thread {
	 ...
	 * entries for.
	 */
	protected FileSystemCheckerJob(final FileSystemChecker givenFileSystemChecker) {
	 ...
	 */
	protected FileSystemCheckerJob(final FileSystemChecker givenFileSystemChecker, final String name) {
	...
	public final void run() {
		while (!stopped) {
			ResolvableFileBean rootElement = fileSystemChecker.getNextElementToIndex();
			//Fill children of the root element if not already done.
			if (rootElement != null) {
				rootElement.getChildren();
				if (!stopped) {
					ResolvableFileBean updateElement = rootElement.getNextDescendantToCheck();
