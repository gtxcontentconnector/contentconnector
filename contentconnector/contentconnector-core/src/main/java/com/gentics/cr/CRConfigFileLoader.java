...
public class CRConfigFileLoader extends CRConfigUtil {
	 ...
	 * configuration.
	 */
	public CRConfigFileLoader(final String name, final String givenWebapproot, final String subdir) {
		super();
		...
		loadConfigFile(CONFIG_DIR + subdir + name + ".properties");
	...
	private void checkForSanity() {
		...
		if (confDir.exists() && confDir.isDirectory()) {
			...
			String errMsg = "CONFIGURATION ERROR: Configuration directory does" + " not seem to contain a"
	...
	private void loadConfigFile(final String path) {
		String errorMessage = "Could not load configuration file at: " + CRUtil.resolveSystemProperties(path) + "!";
		try {
	 ...
	 */
	public static void loadConfiguration(final GenericConfiguration emptyConfig, final String path,
			final String webapproot) throws IOException {
	 ...
	 * @throws IOException - if configuration file cannot be read.
	 */
	public static void loadConfiguration(final GenericConfiguration emptyConfig, Properties props,
	...
	private static void setProperty(final GenericConfiguration config, final String key, final String value,
		...
		//be used
		if (webapproot != null) {
			resolvedValue = resolvedValue.replaceAll("\\$\\{webapproot\\}", webapproot.replace('\\', '/'));
		}
		//Set the property
